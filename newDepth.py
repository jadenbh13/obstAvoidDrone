...
if __name__ == "__main__":
    ...
    while True:
        ...
        if frame != None:
            ...
            amplitude_buf*=(2048/1024)
            amplitude_buf = np.clip(amplitude_buf, 0, 255)
            newAmp = 255 - np.uint8(amplitude_buf)

            cv2.imshow("preview_amplitude", np.uint8(amplitude_buf))

            result_image = process_frame(depth_buf,amplitude_buf)
            result_image = cv2.applyColorMap(result_image, cv2.COLORMAP_AUTUMN)
            ...
            redIm = result_image[:,:,1]
            redIm = 255 - redIm
            redIm = np.uint8(redIm)
            cv2.imshow("red",newAmp)
            #bl = blur(redIm)
            #print(bl[0])
