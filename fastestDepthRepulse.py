...
def mainMethod(newImageXCurr, yPix, radSize, addFactor, seek_points, originPoint, x0, z0):
    ...
    for xPix in range(0, len(newImageXCurr[yPix])):
        ...
        xErr = (len(newImageXCurr[yPix]) * 0.5) - xPix
        xAdd = zRadius * 1.0
        if xErr < 0.0:
            xAdd *= (-1.0)
        elif xErr > 0.0:
...
def getLeanX(newImageList, radSize, addFactor, seek_points, originPoint, v0, v1):
    ...
    procList = []
    #print("Stage3")
    print(len(newImageXs))
    print(len(newImageYs))
...
def getLeanMaster(newImageList0, newImageList1, radSize, addFactor, seek_points, originPoint, v0, v1, v2, v3):

    goalX = int(dataSize * 0.5)
    goal = [goalX,dataSize]
    newImageXs = newImageList0
    newImageYs = newImageList1
    xVector = [0, 0]
    totalXInt = 0
    procList = []
    #print("Stage3")
    ...
    v0.value = xCompensate.value
    v1.value = zCompensate0.value
    v2.value = yCompensate.value
    v3.value = zCompensate1.value
    xCompensate.value = 0
    zCompensate0.value = 0
    yCompensate.value = 0
    zCompensate1.value = 0
...
def getDirection(disps):
    ...
    getLeanMaster(newImageX, newImageY, radSize, addFactor, seek_points, originPoint, xVector0, xVector1, yVector0, yVector1)
    #tm3 = Thread(target=getLeanMaster, args=(newImageX, newImageY, radSize, addFactor, seek_points, originPoint, xVector0, xVector1, yVector0, yVector1))
    #tm3.start()
    #tm3.join()
    """tm1.start()
    tm2.start()
...
def obtainObst(frL, frR):
	...
	disparity_SGBMs = disparity_SGBMs[0:shap[0], 110:shap[1]]
...
heiNum = 30
while True:
    fr0 = cv2.imread('leftImTest.png')
    fr1 = cv2.imread('rightImTest.png')
